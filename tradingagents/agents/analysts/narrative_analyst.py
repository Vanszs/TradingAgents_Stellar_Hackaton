...
def create_news_narrative_analyst(llm):
    ...
    def news_narrative_analyst_node(state):
        ...
        # Fetch news sources
        news_raw = get_news.invoke({"ticker": ticker, "start_date": start_date, "end_date": end_date})
        global_news_raw = get_global_news.invoke({"curr_date": current_date})
        web_search_raw = get_web_search.invoke({"query": f"{ticker} latest narrative market impact {current_date}"})
        ...
        prompt = ChatPromptTemplate.from_messages([
            (
                ...
                "Current analysis date: {current_date}. {instrument_context}" + get_language_instruction(),
            ),
...
def create_market_narrative_analyst(llm):
    ...
    def market_narrative_analyst_node(state):
        ...
        stock_data_raw = get_stock_data.invoke({"symbol": ticker, "start_date": start_date, "end_date": current_date})
        
        # Fetch Indicators
        macd_raw = get_indicators.invoke({"symbol": ticker, "indicator": "macd", "curr_date": current_date, "look_back_days": 60})
        rsi_raw = get_indicators.invoke({"symbol": ticker, "indicator": "rsi", "curr_date": current_date, "look_back_days": 30})
        boll_raw = get_indicators.invoke({"symbol": ticker, "indicator": "boll", "curr_date": current_date, "look_back_days": 30})
